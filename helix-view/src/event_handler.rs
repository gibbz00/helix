use crate::{input::KeyEvent, keyboard::KeyCode, document::Mode, keymap::{KeyTrie, KeyTrieNode}};
...
impl EventHandler {
    ...
    pub fn handle_key_event(&mut self, mode: Mode, key_event: KeyEvent) -> Option<KeyTrieNode::Commands> {
        let mut key_trie_path = self.pending_keys.into_flattened();
        key_trie_path.push(key_event.clone());
        let found_key_trie = keymap.get_keytrie(mode).traverse(key_trie_path);
        ...
        match found_key_trie { 
            ...
            KeyTrieNode::KeyTrie(keytrie) => {
                self.pending_keys.last_mut().push(key);
